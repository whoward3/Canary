...
class CanaryDriver():
 ...
 def start(self,trials):
    ...
    """
    try:
     # start core process, dumps an warning if the core-daemon is already running
     ...
     # initalize canary composition local variable for changes in canary deployment
     n_temp = 0
     ...
     for i in range(trials):
      ...
      if(i%15==0):
        ...
        # after the completion of a composition loop reduce the number of canaries
        if(c == 0 and n_temp <= s):
          n = s - n_temp
          n_temp +=1
        elif(n_temp > s):
      ...
      canary_daemon = subprocess.Popen(["python3","canary_shark.py","-t",str(t),"-n",str(n),"-a",str(a),"-l",str(l)])

      # allow the trial to safely complete
      time.sleep(340)
      print("DRIVER: Trial {}/{}/{} complete! Begining cleanup...".format(i+1,trials,l))
      try:
